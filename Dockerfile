...
FROM golang:1.15-aipine3.12 AS gobuilder-stage
...
# GO언어 환경 변수를 지정하고 /usr/local/bin 경로에 gostart 실행 파일을 생성한다.
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o /usr/local/bin/gostart

# 두 번째 단계다. 두 번째 Dockerfile를 작성한 것과 같다. 베이스 이미지를 작성한다.
# 마지막은 컨테이너로 실행되는 단계이므로 일반적으로 단계명을 명시하지 않는다.
FROM scratch AS runtime-stage

# 첫 번쨰 단계의 이름을 --From 옵션에 넣으면 해당 단계로부터 파일을 가져와서 복사한다.
